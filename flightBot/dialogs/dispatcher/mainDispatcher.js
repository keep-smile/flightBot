...
const {TicketListDialog, HelpDialog, AdminDialog, TicketBuyDialog} = require('../../dialogs');
...
const TICKET_BUY_PROPERTY = 'ticketBuyProperty';
...
module.exports = {
  MainDispatcher: class extends ComponentDialog {
    ...
    constructor(botConfig, onTurnAccessor, conversationState, userState) {
      ...
      this.ticketBuyAccessor = conversationState.createProperty(TICKET_BUY_PROPERTY);
    ...
    async beginChildDialog(dc, onTurnProperty) {
      ...
      switch (onTurnProperty.intent) {
        ...
        case TicketListDialog.Name:
          return await dc.beginDialog(TicketListDialog.Name);
        case TicketBuyDialog.Name:
          return await dc.beginDialog(TicketBuyDialog.Name);
        case AdminDialog.Name:
          // return await this.beginAdminDialog(dc, onTurnProperty);
          return await dc.beginDialog(AdminDialog.Name);
        case NONE_INTENT:
        default:
          return await dc.context.sendActivity(`I'm still learning... Sorry, unrecognized intent. Lets Try again with other words or Cancel? `);
    ...
    async isRequestedOperationPossible(activeDialog, requestedOperation) {
      ...
      if (activeDialog !== undefined && activeDialog.id !== undefined) {
        ...
        if (requestedOperation === AdminDialog.Name) {
          if (activeDialog.id === TicketBuyDialog.Name) {
     ...
     * @param {OnTurnProperty} onTurnProperty
     */
    async beginAdminDialog(dc, onTurnProperty) {
